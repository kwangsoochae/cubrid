...
-- SELECT * FROM DBLINK ('localhost:33000:testdb4dblink:cubrid:cubrid:','SELECT id from remote_t') AS t(id int);
...
-- SELECT l.id, l.name, r.name FROM local_t l, remote_t@cubrid_conn r WHERE l.id = r.id;
...
-- SELECT l.id, l.name, r.name FROM local_t l LEFT JOIN remote_t@cubrid_conn r ON l.id = r.id;

-- 5) 로컬만 조인 결과 (기준 비교용): 원격 테이블을 로컬에 복사해 둔 경우 동일 결과인지 비교
-- SELECT l.id, l.name, r.name FROM local_t l, remote_t r WHERE l.id = r.id;

-- 실행용 (cubrid_conn 사용 시 주석 해제 후 사용)
-- 옵티마이저가 비용 기반으로 조인 순서 결정 (힌트 없음)
SELECT l.id, l.name, r.name FROM local_t l, remote_t@cubrid_conn r WHERE l.id = r.id;
