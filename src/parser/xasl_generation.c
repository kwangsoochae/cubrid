...
parser_generate_xasl_proc (PARSER_CONTEXT * parser, PT_NODE * node, PT_NODE * query_list)
{
  ...
  if (xasl)
      ...
      if ((PT_IS_QUERY (node) && node->info.query.correlation_level == 0) || node->node_type == PT_CTE)
	{
       ...
       * NOTE: if a future join push-down optimization places per-row host variables into conn_sql,
       * this flag must NOT be set for that case; re-bind + re-execute would be required instead. */
