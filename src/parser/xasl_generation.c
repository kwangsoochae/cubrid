 ...
 * fragment (NULL if there are none); the pushable predicate (pt_dblink_delete_is_pushable_pred) is
...
pt_to_delete_xasl_remote_subquery (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  PT_DBLINK_INFO *pdblink = NULL;
  PT_NODE *cond, *pushable, *arg1, *arg2;
  ...
  /* locate the pushable predicate within cond (which may be that predicate alone, or combined with
