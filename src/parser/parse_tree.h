...
struct pt_sp_stmt_info
{
  ...
  PT_NODE *expr3;		/* step of a FOR range, the expression after BY; NULL when none was written */
...
struct parser_context
{
  ...
  struct
  {
    ...
    unsigned is_plcsql_native_exec:1;	/* the static SQL is being read for a plan the server runs, not for the PL
					 * engine's compiler. Both read it the same way; where the two differ is
					 * what becomes of an INTO clause */
