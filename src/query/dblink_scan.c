...
/* Bind a single DB_VALUE to CCI stmt param (shared by host_vars and join_key) */
static int
dblink_bind_one_param (int stmt_handle, int param_pos, DB_VALUE * dbval)
{
  int ret, num_size = 0;
  T_CCI_A_TYPE a_type;
  T_CCI_U_TYPE u_type;
  ...
  unsigned char type = dbval->domain.general_info.type;
  ...
  ret = cci_bind_param (stmt_handle, param_pos, a_type, value, u_type, 0);
  if (ret < 0)
    {
...
dblink_bind_param (int stmt_handle, VAL_DESCR * vd, DBLINK_HOST_VARS * host_vars)
{
  ...
  for (n = 0; n < host_vars->count; n++)
    {
      i = host_vars->index[n];
      ret = dblink_bind_one_param (stmt_handle, n + 1, &vd->dbval_ptr[i]);
      if (ret != NO_ERROR)
	{
	  return ret;
	}
 ...
 *   return: SCAN_CODE
 *   scan_info(in)      : information for dblink
 *   vd(in)             : value descriptor (for join key bind when join_key_count > 0)
 */
SCAN_CODE
dblink_scan_reset (DBLINK_SCAN_INFO * scan_info, VAL_DESCR * vd)
{
  T_CCI_ERROR err_buf;
  T_CCI_CUBRID_STMT stmt_type;
  int ret;
  regu_variable_list_node *regu_node;
  DB_VALUE *dbval;
  ...
  if (scan_info->join_key_count > 0)
    {
      ...
      if (vd == NULL)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_DBLINK, 1, "vd required for join key bind");
	  return S_ERROR;
	}
      /* T3-2: bind join keys from vd, then execute */
      for (regu_node = (regu_variable_list_node *) scan_info->join_key_regu_list; regu_node != NULL;
	   regu_node = (regu_variable_list_node *) regu_node->next, param_pos++)
	{
	  dbval = regu_node->value.vfetch_to;
	  if (dbval == NULL && regu_node->value.type == TYPE_POS_VALUE)
	    {
	      dbval = &vd->dbval_ptr[regu_node->value.value.val_pos];
	    }
	  if (dbval == NULL)
	    {
	      dbval = regu_node->value.value.dbvalptr;
	    }
	  if (dbval == NULL
	      && (regu_node->value.type == TYPE_ATTR_ID || regu_node->value.type == TYPE_CLASS_ATTR_ID
		  || regu_node->value.type == TYPE_SHARED_ATTR_ID))
	    {
	      dbval = regu_node->value.value.attr_descr.cache_dbvalp;
	    }
	  if (dbval == NULL)
	    {
	      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_DBLINK, 1, "join key value not resolved");
	      return S_ERROR;
	    }
	  ret = dblink_bind_one_param (scan_info->stmt_handle, param_pos, dbval);
	  if (ret != NO_ERROR)
	    {
	      return S_ERROR;
	    }
